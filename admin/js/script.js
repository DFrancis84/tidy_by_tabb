...
const demo=[
{id:"g1",title:"Kitchen Deep Clean",category:"Kitchen",beforeImage:"",afterImage:"",comparisonImage:"",status:"published",featured:true,createdAt:new Date().toISOString(),updatedAt:new Date().toISOString()},
{id:"g2",title:"Bathroom Refresh",category:"Bathroom",beforeImage:"",afterImage:"",comparisonImage:"",status:"draft",featured:false,createdAt:new Date().toISOString(),updatedAt:new Date().toISOString()}
];
let records=load();
...
function openDrawer(id=""){
 ...
 preview($("#beforeImage"),$("#beforePreview"));preview($("#afterImage"),$("#afterPreview"));
 drawer.classList.add("open");drawer.setAttribute("aria-hidden","false");overlay.classList.add("open")
}
function closeDrawer(){drawer.classList.remove("open");drawer.setAttribute("aria-hidden","true");overlay.classList.remove("open");form.reset()}
function togglePublish(id){const r=records.find(x=>x.id===id);if(!r)return;r.status=r.status==="published"?"draft":"published";r.updatedAt=new Date().toISOString();save();toast(r.status==="published"?"Published":"Returned to draft")}
function removeCurrent(){const id=$("#recordId").value;if(!id)return;if(!confirm("Delete this transformation?"))return;records=records.filter(r=>r.id!==id);save();closeDrawer();toast("Transformation deleted")}
form.onsubmit=e=>{e.preventDefault();const id=$("#recordId").value||`g_${Date.now()}`,existing=records.find(r=>r.id===id),featured=$("#recordFeatured").checked;
 if(featured)records.forEach(r=>r.featured=false);
 const item={id,title:$("#recordTitle").value.trim(),category:$("#recordCategory").value,beforeImage:$("#beforeImage").value.trim(),afterImage:$("#afterImage").value.trim(),comparisonImage:$("#comparisonImage").value.trim(),status:$("#recordStatus").value,featured,createdAt:existing?.createdAt||new Date().toISOString(),updatedAt:new Date().toISOString()};
 if(existing)records=records.map(r=>r.id===id?item:r);else records.unshift(item);
 save();closeDrawer();toast(existing?"Transformation updated":"Transformation added")
};
$$(".nav").forEach(n=>n.onclick=()=>show(n.dataset.view));$$("[data-go]").forEach(b=>b.onclick=()=>show(b.dataset.go));
menu.onclick=()=>{sidebar.classList.toggle("open");overlay.classList.toggle("open")};
overlay.onclick=()=>{closeSidebar();closeDrawer()};
$("#addTransformation").onclick=()=>openDrawer();$("#closeDrawer").onclick=closeDrawer;$("#cancelEdit").onclick=closeDrawer;$("#deleteRecord").onclick=removeCurrent;
